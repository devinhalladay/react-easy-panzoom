...
class PanZoom extends React.Component<Props, State> {
  ...
  onWheel = (e: WheelEvent) => {
    ...
    let zoomEndCallback = () => this.triggerOnZoomEnd(e);

    window.clearTimeout(this.zooming);
  ...
  triggerOnZoomEnd = (e: WheelEvent) => {
    const { disableScrollZoom, disabled, onZoomEnd } = this.props;
    if (disableScrollZoom || disabled) {
      return;
    }
